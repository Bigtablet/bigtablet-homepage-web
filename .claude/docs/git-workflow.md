...
조직 공용 이슈 템플릿(GitHub 조직 레벨 제공)을 사용합니다. 작업 성격에 맞는 템플릿을 골라 **제목 접두사 · 라벨 · 섹션**을 맞춥니다. Issue 본문은 한글로 작성합니다.
...
> **⚠️ 자의적으로 섹션을 변경하거나 추가하지 마세요. `## 개요`, `## 변경사항`, `## 관련 파일` 등 다른 형식 사용 금지.**

### Issue

조직 공용 템플릿(`[BUG]` / `[FEATURE]` / `[TASK]`)을 사용합니다. 위 "1. Issue 생성" 표의 템플릿별 제목 접두사 · 라벨 · 섹션을 그대로 따릅니다. 체크박스 항목(`TO DO` 등)은 `- [ ]` (미완료).
