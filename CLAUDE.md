...
**"생성"은 완료가 아니다.** 작업을 끝냈다고 보고하기 전에 아래를 대조한다. 단위 작업을 쪼개서 일부만 하고 멈추지 않는다.

- **PR**: 생성 + 라벨 부착 + base `develop` + 본문 템플릿 + `Closes #이슈` + CI 통과 확인 + (리뷰가 달렸으면) **봇 코멘트 전부 대응**.
- **Jira 티켓**: 생성 + **부모 `DEV-388`** + **담당자 본인** + `[WEB]` 접두사 + 필드 4종(설명·기존안·수정안·시작날짜) + 생성 전 중복 검색.
- **리뷰 대응 묶음**: 코멘트 1건당 `[반영 또는 미반영 사유]` + `[작성 봇 멘션 답글]` + `[스레드 resolve]` 를 **한 묶음으로** 처리한다. "수정만" 하고 답글·resolve를 빼먹지 않는다.
- 리뷰는 PR 생성 후 비동기로 도착하므로, 리뷰 이벤트가 오거나 사용자가 "리뷰 체크"를 요청하면 위 묶음을 끝까지 수행한다.
